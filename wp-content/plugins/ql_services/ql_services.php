...
function ql_services_shortcode($atts)
{
	...
	{
		$i=0;
		$divider = ($type=="icons_list" ? 2 : 3);
		$output .= '<ul class="services-list clearfix' . (!empty($type) && $type=="icons_list" ? ' services-icons' : '') . ((int)$borders ? ' services-items-border' : '') . (!empty($top_margin) && $top_margin!="none" ? ' ' . $top_margin : '') . '">';
			...
			if($type=="icons_list")
			{
				if((int)$show_icon)
				{
					...
					if(!empty($icon))
					{
						...
						$output .= '<span class="service-icon sl-small-' . esc_attr($icon) . '"></span>';
						if((int)$icons_links)
							$output .= '</a>';
