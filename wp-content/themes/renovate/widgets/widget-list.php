<?php
class re_list_widget extends WP_Widget 
{
	/** constructor */
    function re_list_widget() 
	{
		$widget_options = array(
			'classname' => 're_list_widget',
			'description' => 'Displays Bulleted List'
		);
		$control_options = array('width' => 625);
        parent::__construct('renovate_list', __('Bulleted List', 'renovate'), $widget_options, $control_options);
    }
	...
	{	
		...
		<p class="widget-border">
			<label for="<?php echo esc_attr($this->get_field_id('list_item')) . $i; ?>"><?php _e('List item text', 'renovate'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('list_item_url')) . $i; ?>"><?php _e('List item url', 'renovate'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('list_item_target')) . $i; ?>"><?php _e('List item url target', 'renovate'); ?></label>
			<select name="<?php echo esc_attr($this->get_field_name('list_item_target')); ?>[]">
				<option value="same_window"<?php echo (isset($list_item_target[$i]) && $list_item_target[$i]=="same_window" ? " selected='selected'" : ""); ?>><?php _e('same window', 'renovate'); ?></option>
				<option value="new_window"<?php echo (isset($list_item_target[$i]) && $list_item_target[$i]=="new_window" ? " selected='selected'" : ""); ?>><?php _e('new window', 'renovate'); ?></option>
			</select>
		</p>
		<?php
		}
...
//register widget
add_action('widgets_init', create_function('', 'return register_widget("re_list_widget");'));
?>