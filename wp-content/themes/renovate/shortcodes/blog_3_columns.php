<?php
//blog
function re_theme_blog_3_columns($atts, $content)
{
	...
	{
		$output .= '<div class="vc_row wpb_row vc_row-fluid layout-' . esc_attr($layout_type) . ($top_margin!="none" ? ' ' . $top_margin : '') . ($el_class!="" ? ' ' . $el_class : '') . '">';
		$i = 0;
		...
		while ($blog_query->have_posts()) : $blog_query->the_post();
			...
			{
				$output .= '<h4 class="box-header align-left"><a href="' . get_permalink() . '" title="' . esc_attr(get_the_title()) . '">' . get_the_title() . '</a></h4>';
			}
	...
	{
		$output .= '<div class="vc_row wpb_row vc_row-fluid">' . sprintf(__('No results found for %s', 'renovate'), get_query_var('s')) . '</div>';
	}
	if($re_pagination)
	{
...
//visual composer
function re_theme_blog_3_columns_vc_init()
{
	//get posts list
	...
	vc_map( array(
		"name" => __("Blog 3 Columns", 'renovate'),
		"base" => "blog_3_columns",
		"class" => "",
		"controls" => "full",
...
add_action("init", "re_theme_blog_3_columns_vc_init");
?>