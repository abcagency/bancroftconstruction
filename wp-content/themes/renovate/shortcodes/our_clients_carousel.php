...
function re_theme_our_clients_carousel($atts, $content)
{
	...
	foreach($images as $attach_id)
	{
		...
		if($onclick=="link_image")
		{
			...
			$output .= '<a href="' . $custom_links[$i] . '"' . ($custom_links_target=="_blank" ? ' target="_blank"' : '') . '>' . wp_get_attachment_image($attach_id, "full", false, array("alt" => "")) . '</a>';
		else
...
class WPBakeryShortCode_Our_Clients_Carousel extends WPBakeryShortCode {
	public function content( $atts, $content = null ) {
        return re_theme_our_clients_carousel($atts);
    }
