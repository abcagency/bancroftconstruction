...
function re_theme_items_list($atts, $content)
{
	...
	$output = '<ul class="list' . ($class!='' ? ' ' . $class : '') . ($top_margin!="none" ? ' ' . $top_margin : '') . '">' . wpb_js_remove_wpautop($content) . '</ul>';
	return $output;
}
...
function re_theme_item($atts, $content)
{
	...
	$output = '<li class="' . ($icon!="" || $class!="" ? ($icon!="" ? 'template-' . $icon . ' ': '') . ($class!="" ? $class . ' ' : '') : '') . '"' . ($text_color!='' ? ' style="' . ($text_color!='' ? 'color:' . $text_color . ';' : '') . '"' : '') . '>' . '<' . ($url!="" ? 'a href="' . esc_attr($url) . '"' . ($url_target=='new_window' ? ' target="_blank"' : '') : 'span') . ($text_color!='' ? ' style="color: ' . $text_color . ';"' : '') . '>' . do_shortcode($content) . '</' . ($url!="" ? 'a' : 'span') . '>';
	$output .= '</li>';
