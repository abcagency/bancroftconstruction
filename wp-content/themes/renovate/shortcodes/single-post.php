<?php
//post
function re_theme_single_post($atts, $content)
{
	global $themename;
	...
	if((int)$show_post_date || (int)$show_post_views || (int)$show_post_comments)
	{
		...
		if((int)$show_post_comments)
		{
			...
			$output .= '<li class="template-bubble"><a class="scroll-to-comments" href="' . get_comments_link() . '" title="' . $comments_count . ' ' . ($comments_count==1 ? __('comment', 'renovate') : __('comments', 'renovate')) . '">' . $comments_count . '</a></li>';
		}
		$output .= '</ul>';
	}
	...
	if((int)$show_post_author || (int)$show_post_categories)
	{
		...
		{
			$output .= '<li>' . __("By ", 'renovate') . '<a href="' . get_author_posts_url(get_the_author_meta('ID')) . '" title="' . esc_attr(get_the_author()) . '">' . get_the_author() . '</a></li>';
		}
	...
	if($show_post_title) 
		$output .= '<h2 class="box-header align-left"><a href="' . get_permalink() . '" title="' . esc_attr(get_the_title()) . '">' . get_the_title() . '</a></h2>';
