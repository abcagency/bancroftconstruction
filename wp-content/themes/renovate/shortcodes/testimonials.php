...
function renovate_testimonials_shortcode($atts)
{
	...
	$output .= '<ul class="testimonials-list autoplay-' . $autoplay . ' pause_on_hover-' . $pause_on_hover . ' scroll-' . $scroll . ' effect-' . $effect . ' easing-' . $newEasing . ' duration-' . $duration . ((int)$ontouch ? ' ontouch' : '') . ((int)$onmouse ? ' onmouse' : '') . '">';
	
	if(is_rtl())
	{
		for($i=$testimonials_count-1; $i>=0; $i--)
		{
			$output .= '<li' . (!empty($atts["testimonials_icon" . $i]) && $atts["testimonials_icon" . $i]!="" && $atts["testimonials_icon" . $i]!="none" && (empty($type) || (!empty($type) && $type=="big")) ? ' class="sl-small-' . $atts["testimonials_icon" . $i] . '"' : '') . '>
					...
					' . (!empty($type) && $type=="small" ? '<div class="ornament' . (!empty($atts["testimonials_icon" . $i]) && $atts["testimonials_icon" . $i]!="" && $atts["testimonials_icon" . $i]!="none" ? ' sl-small-' . $atts["testimonials_icon" . $i] : ' empty-circle') . '"></div>' : '') . '
