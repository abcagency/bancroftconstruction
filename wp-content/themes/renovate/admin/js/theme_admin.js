jQuery(document).ready(function($){
	...
	//upload
	$("[name='renovate_upload_button']").live('click', function(){
		var self = $(this);
	...
	$(".add_new_repeated_row").click(function(){
	...
	$("#import_dummy").click(function(event){
		...
		$.ajax({
				...
				type: 'post',
				//dataType: 'json',
				data: "action=renovate_import_dummy",
				success: function(json){
					json = $.trim(json);
					...
					$.ajax({
							...
							type: 'post',
							//dataType: 'json',
							data: "action=renovate_import_dummy2",
	...
	$("#import_shop_dummy").click(function(event){
		...
		$.ajax({
				...
				type: 'post',
				//dataType: 'json',
				data: "action=renovate_import_shop_dummy",
				success: function(json){
					json = $.trim(json);
					var indexStart = json.indexOf("dummy_import_start")+18;
					var indexEnd = json.indexOf("dummy_import_end")-indexStart;
	...
	$("#primary_font").change(function(event, param){
		...
		if(self.val()!="")
		{
			...
			$.ajax({
					url: ajaxurl,
					type: 'post',
					data: "action=renovate_get_font_subsets&font=" + $(this).val(),
					success: function(data){
						data = $.trim(data);
						var indexStart = data.indexOf("re_start")+8;
						var indexEnd = data.indexOf("re_end")-indexStart;
